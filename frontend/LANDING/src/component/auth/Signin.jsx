    ...
    headers: {
      ...
      if (response.ok) {
       ...
       // window.location.href = `/Admin/${username}`;
        // if(jso.typeus === 'Admin'){
        //   localStorage.setItem("token", JSON.stringify(jso));
        //      let red=await fetch( `http://localhost:4000/api/authentification/admin/${values.username}`)
             
        //      }
        if (jso.typeus === 'Admin') {
          localStorage.setItem("token", JSON.stringify(jso));
          localStorage.setItem("Login", JSON.stringify(true));
          let response = await fetch(`http://localhost:4000/api/authentification/profile/${values.username}`);
